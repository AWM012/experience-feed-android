...
本项目参考抖音 App 首页上方「经验」入口的内容形态，实现一个 Android 图文浏览应用，核心目标是串联双列图片流、图文详情页、网络图片加载、实时天气和本地缓存复用。
...
本项目的 UI 目标不是做复杂动效，而是让核心功能表达清楚：

- 首页和详情页显示「首次网络下载 / OkHttp 响应缓存 / 本地磁盘缓存 / 当前内存缓存」，方便观察不同层级的缓存命中。
- 详情页展示缓存文件路径，说明图片确实已经落到本地。
- 详情页提供「测试 OkHttp 响应缓存」按钮，清除图片文件缓存但保留 HTTP 缓存，用于观察 OkHttp 缓存命中。
...
- 不使用 RecyclerView，是为了让双列分配算法更直观，便于理解布局过程。
- 使用原生 Activity 和 Java，是为了降低额外框架干扰，突出 Android 基础能力。
